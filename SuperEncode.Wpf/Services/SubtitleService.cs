...
public class SubtitleService
{
        ...
        try
        {
            while (!cancellation.IsCancellationRequested)
            {
                if (subtitleSetting.SubtitleInFile)
                    subtitlePath = await ExportSubtitleFromVideo(path, false);
                else
    ...
    private async Task<string> ScanSubtitleFromFolder(string inputFile, string suffixString)
    {
        ...
        foreach (var subtitleFileName in suffixList.Select(suffixFileName =>
                     $"{fileNameWithoutExtension}{suffixFileName}{subtitleExtension}"))
        {
    ...
    {
        var actuallySubtitleFullPath =
            Path.Combine(Path.GetTempPath(), Path.ChangeExtension(Path.GetTempFileName(), ".ass"));
        ...
        var assSubContent =
            new AdvancedSubStationAlpha().ToText(subtitleEdit, Path.GetFileName(actuallySubtitleFullPath));
    ...
    public async Task UpdateAssStyle(string filePath, SubtitleSetting subtitleSetting)
    {
        ...
        outputContent = outputContent
            ...
            .Replace("[[[Marquee-FontSize]]]",
                (subtitleSetting.FontSize * 7.5 / 10).ToString(CultureInfo.InvariantCulture));
        ...
        if (string.IsNullOrWhiteSpace(subtitleSetting.Marquee))
        {
            ...
            if (beginMarqueeIndex > -1) outputContent = outputContent[..(beginMarqueeIndex - 1)];
        }
        ...
        var endEventIndex = inputContent.IndexOf("\n", beginEventIndex + beginEventString.Length,
        ...
        if (subtitleSetting.OverrideStyleDefault)
        {
            ...
            if (defaultStyle != null) inputStyleArray.Remove(defaultStyle);
        }
