...
public partial class UcVideoPlayer
{
    ...
    public static readonly DependencyProperty SubtitleUrlProperty =
        DependencyProperty.Register(
            ...
            new FrameworkPropertyMetadata(OnSubTitleVideoUrlChanged));
    ...
    public static void OnSubTitleVideoUrlChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        {
            if (ucVideoPlayer._mediaPlayer.IsPlaying) ucVideoPlayer._mediaPlayer.Stop();
            return;
        }

        ucVideoPlayer.VideoView.Background = new SolidColorBrush(Color.FromRgb(0, 0, 0));


        ucVideoPlayer.LoadMedia(newSubtitleUrl);
    }

    private async void LoadMedia(string subtitleUrl)
    {
        if (!File.Exists(subtitleUrl)) return;
    ...
    private void BtnStop_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        VideoView.Background = new SolidColorBrush(Color.FromRgb(0, 0, 0));
    }
    ...
    {
        var mediaPlayer = VideoView.MediaPlayer;
        if (mediaPlayer != null) mediaPlayer.Time = (long)e.NewValue;
    }
}